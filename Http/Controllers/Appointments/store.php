...
if (!Validator::email($email)) {
 ...
 if (! empty($errors)) {
    return views('Appointment/Book_Appointment.view.php', [
        'errors' => $errors
    ]);
}
$required_fields = [
...
if ($userAppointment) {
    die('<script>alert("You already have an appointment for this day. Please reschedule or edit your existing appointment."); window.history.back();</script>');
}
...
if ($user) {
  ...
  $mail->Username = $_ENV['SMTP_USERNAME'] ?? 'YOUR SMTP USERNAME'; 
        $mail->Password = $_ENV['SMTP_PASSWORD'] ?? 'YOUR SMTP PASSWORD';        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port = 587;
